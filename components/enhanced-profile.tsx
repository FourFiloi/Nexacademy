      ...
      >
        <div className="flex flex-col gap-6 md:flex-row md:items-start">
          ...
          <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4 md:mt-0 md:ml-auto">
            <div className="flex flex-col items-center rounded-lg bg-secondary/30 p-3">
              ...
              <span className="text-xl font-bold">{userData.currentXP.toLocaleString()}</span>
              <span className="text-xs text-muted-foreground">Total XP</span>
            </div>

            <div className="flex flex-col items-center rounded-lg bg-secondary/30 p-3">
              <BookOpen className="h-5 w-5 text-blue-500 mb-1" />
              <span className="text-xl font-bold">{oldUserData.stats.coursesCompleted}</span>
              <span className="text-xs text-muted-foreground">Courses</span>
            </div>

            <div className="flex flex-col items-center rounded-lg bg-secondary/30 p-3">
              <FileText className="h-5 w-5 text-green-500 mb-1" />
              <span className="text-xl font-bold">{oldUserData.stats.assignmentsCompleted}</span>
              <span className="text-xs text-muted-foreground">Assignments</span>
            </div>

            <div className="flex flex-col items-center rounded-lg bg-secondary/30 p-3">
              <Code className="h-5 w-5 text-purple-500 mb-1" />
              <span className="text-xl font-bold">{oldUserData.stats.problemsSolved}</span>
        ...
        <div className="mt-6">
          ...
          <div className="mt-2 flex items-center gap-3">
            <Progress value={userData.level} className="h-2 flex-1" />
      ...
      >
        ...
        <div className="mt-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 text-sm">
          <div className="flex items-center gap-2">
            ...
            <span className="text-muted-foreground">· {oldUserData.stats.daysActive} active days this year</span>
          </div>
          <div className="text-muted-foreground">
            {oldUserData.heatmapData.reduce((sum, day) => sum + day.count, 0)} total activities in the last year
          </div>
        </div>
      </motion.div>
      ...
      >
        <Tabs defaultValue="overview" value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="overview" className="p-6">
            <ProfileOverview user={oldUserData} />
          </TabsContent>

          <TabsContent value="badges" className="p-6">
            <ProfileBadges badges={oldUserData.badges} />
          </TabsContent>
